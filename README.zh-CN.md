...
<p align="center">一套面向所有 AI 智能体的通用工作流标准：<b>流程全局定、状态跟项目根；开工必读状态，收工必留锚点</b>。让任何 AI 会话都能无痛接续，避免单会话拉长而失忆。</p>
...
- **状态跟着项目根走**：`.workflow` 只放项目根，不放源码目录，也不放工作区根；已有 `.workflow` 就近沿用、不自动迁移。
...
| **路径判定自动化** | 用户指定 → 向上查找已有 `.workflow` → 再判定项目根 / 源码目录 / 工作区；全程不写死路径 |
...
**先分清三个概念：项目根是单个项目的管理与交付边界，也是 `.workflow` 的唯一锚点；源码目录只是项目内部的代码位置，可以有独立 `.git`；工作区根只是并列多个项目的父目录。**

| 目录 | 定义 | `.workflow` 放哪 |
|---|---|---|
| **项目根目录** | 单个项目的管理边界；可以没有 Git，也可以与源码目录重合 | `<项目根>\.workflow\` |
| **源码目录** | 实际代码 / 仓库位置；可以是项目根子目录 | 不放，除非它同时被明确认定为项目根 |
| **工作区根目录** | 并列多个项目根的工作区容器 | 不放；先选具体项目，再放到该项目根下 |

```text
<工作区根>\
├── <项目甲>\          # 项目根；.workflow 放这里
│   ├── .workflow\
│   └── src\           # 源码目录；可以有自己的 .git
└── <项目乙>\          # 另一个项目根
    ├── .workflow\
...
1. 用户明确指定项目根 → 直接用。
2. 从 cwd 向上查找已有的 `.workflow\STATE.md` → 找到就沿用，**永不自动迁移**。
3. 没有已有状态时，再判定项目根 / 源码目录 / 工作区根；`.git` 只是辅助线索，不能单独决定项目根。
4. 无法确定时先问，不在源码目录或工作区根乱建 `.workflow`。

> 项目根与源码目录重合也是合法形态（单仓库项目）；关键是状态锚点始终跟着项目根，而不是跟着任意一个 Git 仓库。
...
- **状态目录在哪？** 先从 cwd 向上找已有 `.workflow\`；没有时再按路径模型确定项目根，状态始终放 `<项目根>\.workflow\`，不放源码目录或工作区根。
- **多个智能体状态不同步？** 确认它们指向同一项目目录（同一份 `.workflow\`）。本技能设计为共享一份状态；若各自建了 `.workflow`，说明项目目录不一致。
- **源码目录里有 `.git`，状态应该放那里吗？** 不应只凭 `.git` 判断。源码仓库可以是项目根的子目录；若已有 `.workflow` 在上层或用户已指定项目根，就沿用项目根。
