...
**先分清三个概念：项目根是拥有整个项目状态的目录，也是 `.workflow` 的唯一锚点；源码目录只是项目内部的代码位置，可以有独立 `.git`；工作区根只是并列多个项目根的父目录。**

| 目录 | 定义 | `.workflow` 放哪 |
|---|---|---|
| **项目根目录** | 拥有整个项目状态的目录；可以没有 Git，也可以与源码目录重合 | `<项目根>\.workflow\` |
| **源码目录** | 存放代码 / 仓库的目录；只是项目内部位置 | 不放，除非用户明确说明它同时就是项目根 |
