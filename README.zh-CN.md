...
**先分清三个概念：项目根是拥有整个项目状态的目录，也是 `.workflow` 的唯一锚点；源码目录默认是项目根下的代码子目录；工作区根只是并列多个项目根的父目录。**

| 目录 | 定义 | `.workflow` 放哪 |
|---|---|---|
| **项目根目录** | 拥有整个项目状态的父目录；默认承载 `.workflow` 和源码目录 | `<项目根>\.workflow\` |
| **源码目录** | 项目根下的代码目录；可以有独立 `.git` | 不放，除非用户明确说明它同时就是项目根 |
| **工作区根目录** | 并列多个项目根的容器；不拥有某个项目的状态 | 不放；先选具体项目根 |

```text
<项目根目录>\
├── .workflow\         # 工作流；直接放在项目根
└── <源码目录>\        # 源码目录；位于项目根之下
```
...
1. 用户明确指定项目根 → 直接用。
2. 从 cwd 向上查找已有的 `.workflow\STATE.md` → 找到最近的，其父目录就是项目根；**永不自动迁移**。
3. 两种证据都没有 → 先问“项目根目录是哪一个？”，不得用 `.git`、`package.json`、`src`、`README`、cwd 或目录结构自行猜测。

> 默认项目根与源码目录分开；只有用户明确说明“源码目录同时就是项目根”时才重合。关键是状态锚点始终跟着项目根，而不是跟着任意一个 Git 仓库。
...
- **状态目录在哪？** 先认用户明确指定的项目根，再从 cwd 向上找已有 `.workflow\`；两者都没有就先问。状态始终放 `<项目根>\.workflow\`，不放源码目录或工作区根。
