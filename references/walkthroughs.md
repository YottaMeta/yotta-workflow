# 元序复杂场景走查

## 走查 1：项目开工

1. 先看用户是否明确指定项目根；再看 cwd 向上是否有已有 `.workflow\STATE.md`。
2. 两者都没有时停止初始化，先问“项目根目录是哪一个？”；不得用 `.git`、`package.json`、`src`、`README` 或 cwd 猜。
3. 读取项目根下 `.workflow\` 的 STATE / TASKS / ROADMAP / DECISIONS。
4. 汇总当前进度与下一步。
5. 执行前先确认本次会话目标与验收。

## 走查 2：任务中主动落盘

背景：一个功能已改完一半，后续还有测试和文档。

1. 立刻更新 STATE 当前进度。
2. 在 TASKS 勾选已完成项。
3. 如有新决策，写入 DECISIONS。
4. 当天日志追加一条简短记录。
5. 继续执行剩余步骤。

## 走查 3：收工锚点

1. 更新 STATE / TASKS / ROADMAP。
2. 追加当天日志。
3. 输出固定格式锚点，包含项目、项目根绝对路径、时间、进度、已完成、下一步、关键决定、遗留问题、开工读取路径。
4. 确认锚点与状态文件一致。

## 走查 4：源码目录里有 `.git`，项目状态该放哪？

背景：当前 cwd 是 `D:\work\my-project\src`，`src` 有 `.git`；用户此前给的项目根是 `D:\work\my-project`。

1. 用户指定的项目根优先，不因 cwd 或 `.git` 改变。
2. 检查 `D:\work\my-project\.workflow\`；存在则沿用，不存在则在项目根初始化。
3. `src` 只作为源码目录，不在 `src\.workflow\` 另建状态。
