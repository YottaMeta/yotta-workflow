# 元序异常恢复手册

## 1. 状态文件缺失

- 信号：项目存在但状态目录不存在。
- 动作：按项目根初始化标准结构，不覆盖既有机制。
- 恢复：读取项目文档与用户补充信息，重建最小状态。

## 2. 并发写入冲突

- 信号：两个会话同时更新同一状态文件。
- 动作：停止整文件重写，改为读取后合并。
- 恢复：保留双方有效信息，冲突处标注来源并请用户确认。

## 3. 状态文件过大

- 信号：单文件明显超过常规阅读量。
- 动作：拆分历史内容，保留当前快照。
- 恢复：STATE 只留当前进度，历史写入日志或专项文档。

## 4. 既有机制冲突

- 信号：项目已有自己的交接文件。
- 动作：沿用原机制，只补足开工读取与收工锚点。
- 恢复：把本协议必要字段映射到原机制，不重复建一套状态。

## 5. 会话中断

- 信号：用户直接开新会话，未走收工流程。
- 动作：先读状态，再请用户补充缺失点。
- 恢复：确认后从下一步继续，不假装记得对话内容。

## 6. 误把源码目录当项目根

- 信号：某个子目录有 `.git`，AI 在它下面新建了 `.workflow`，但上层其实已有项目根或状态。
- 动作：立即停止新增写入；先检查上层已有 `.workflow` 和用户指定的项目根；不要自动移动或合并文件。
- 恢复：确认项目根后，由用户确认如何处理误建的 `.workflow`；后续状态只写入正确项目根，已有正确 `.workflow` 保持原位。
