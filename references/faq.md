...
状态目录固定为 `<项目根目录>\.workflow\`。定位顺序是：用户明确指定的项目根 → 从 cwd 向上查找已有 `.workflow` → 再区分项目根、源码目录和工作区根。已有 `.workflow` 永不自动迁移。
...
| 名称 | 含义 | `.workflow` |
| --- | --- | --- |
| 项目根目录 | 单个项目的管理与交付边界 | 放这里 |
| 源码目录 | 代码 / 资源 / 某个仓库所在位置 | 不放，除非它同时被明确认定为项目根 |
| 工作区根目录 | 并列多个项目的父目录 | 不放；先选具体项目根 |
