...
```text
D:\AI_WorkDir\项目\<项目名>\             # 项目根目录
├── .workflow\                           # 工作流
└── <源码目录>\                          # 源码目录
```

默认情况下，**项目根与源码目录分开**；只有用户明确说明“源码目录同时就是项目根”时，两者才重合。源码目录是否放 `.workflow`，只看它是否被明确认定为项目根，不看它是不是 Git 仓库。
...
如果用户只给了源码目录，却没有指定项目根、也没有已有 `.workflow`，默认继续问它的父级项目根；除非用户明确说“这个源码目录同时就是项目根”。
...
| 场景 | 项目根 | 源码目录 | 状态目录 |
| --- | --- | --- | --- |
| 标准形态：项目根下放源码目录 | `D:\work\my-project` | `D:\work\my-project\src` | `D:\work\my-project\.workflow\` |
| 用户明确说源码目录同时也是项目根 | `D:\work\my-app` | `D:\work\my-app` | `D:\work\my-app\.workflow\` |
| 已有状态在项目甲中 | `D:\work\my-project` | `D:\work\my-project\src` | `D:\work\my-project\.workflow\`；不是 `D:\work\.workflow\` |
| 没有用户指定，也没有已有 `.workflow` | 先问 | 先问 | 不创建 |
