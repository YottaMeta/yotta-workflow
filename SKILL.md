...
> 说明：**状态目录**是项目内的隐藏目录（名称以 `.` 开头，一般为所用智能体的名称，如 `.codex`、`.cursor`、`.opencode` 等），安装到具体智能体后按实际名称替换。
...
- `base` **本身就是一个项目根目录**（含项目标识，如 `.git`、项目配置、或用户明确指向的单一项目）→ 状态文件放 `base` 下 `.<状态目录>\`
  ```
  <base>\.<状态目录>\
  ├── STATE.md
  ├── TASKS.md
  ...
  └── logs\
      └── YYYY-MM-DD.md
  ```
- `base` 是**工作区根目录**（下面并列多个项目子目录）→ 按项目名建子目录：`<base>\<项目名>\.<状态目录>\`（结构同上）。

判定口诀：**先取 cwd，再看它是不是项目根；是项目根就就近存，是工作区就按项目名分开存；全程不写死任何默认/固定路径。**
...
状态文件结构（两种位置下结构一致）。**所有状态文件与流水日志都存放在本项目自己的状态目录下**（位置见「〇」），项目之间互不共享、互不读写：
...
1. 按「〇、状态文件位置判定规则」确定状态目录，检查 `.<状态目录>\STATE.md` 是否存在。
...
开工请先读取：.<状态目录>\STATE.md、TASKS.md、ROADMAP.md（状态目录位置见「〇、判定规则」）
```
